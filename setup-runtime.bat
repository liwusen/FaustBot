...
call :confirm_step "[1/10] Prepare .runtime directory"
if errorlevel 1 goto :step1_skip
echo [1/10] Preparing .runtime directory...
if not exist "%RUNTIME_DIR%" mkdir "%RUNTIME_DIR%"
:step1_skip

call :confirm_step "[2/10] Download Python %PYTHON_VERSION% embedded package"
if errorlevel 1 goto :step2_skip
echo [2/10] Downloading Python %PYTHON_VERSION% embedded package...
...
if not exist "%RUNTIME_DIR%\python-embed.zip" (
  echo Embedded package archive not found. Step [3/10] cannot continue.
  goto :fail
)
call :confirm_step "[3/10] Extract embedded Python"
if errorlevel 1 goto :step3_skip
echo [3/10] Extracting embedded Python...
...
if not exist "%PYTHON_EXE%" (
  echo Python executable not found. Step [4/10] cannot continue.
  goto :fail
)

call :confirm_step "[4/10-A] Enable site-packages for embedded Python"
if errorlevel 1 goto :step4a_skip
echo [4/10-A] Enabling site-packages...
if exist "%PTH_FILE%" (
...
call :confirm_step "[4/10-B] Download get-pip.py"
if errorlevel 1 goto :step4b_skip
echo [4/10-B] Downloading get-pip.py...
...
if not exist "%RUNTIME_DIR%\get-pip.py" (
  echo get-pip.py not found. Step [4/10-C] cannot continue.
  goto :fail
)
call :confirm_step "[4/10-C] Install pip into embedded Python"
if errorlevel 1 goto :step4c_skip
echo [4/10-C] Installing pip...
"%PYTHON_EXE%" "%RUNTIME_DIR%\get-pip.py"
if errorlevel 1 goto :fail
:step4c_skip

call :ensure_pip || goto :fail
call :confirm_step "[5/10] Upgrade pip, setuptools, wheel via Tsinghua PyPI"
if errorlevel 1 goto :step5_skip
echo [5/10] Upgrading base packaging tools via Tsinghua PyPI...
%PIP_CMD% install --upgrade pip setuptools wheel -i %PIP_INDEX_URL%
if errorlevel 1 goto :fail
:step5_skip

call :ensure_pip || goto :fail
call :confirm_step "[6/10] Install PyTorch CUDA 12.8 stack"
if errorlevel 1 goto :step6_skip
echo [6/10] Installing PyTorch CUDA 12.8 stack...
...
call :ensure_pip || goto :fail
call :confirm_step "[7/10] Install project requirements via Tsinghua PyPI"
if errorlevel 1 goto :step7_skip
echo [7/10] Installing project requirements via Tsinghua PyPI...
%PIP_CMD% install -r "%CD%\requirements.txt" -i %PIP_INDEX_URL%
if errorlevel 1 goto :fail
:step7_skip

if not exist "%BACKEND_DIR%\download_tts.py" (
  echo backend\download_tts.py not found. Step [8/10] cannot continue.
  goto :fail
)
call :confirm_step "[8/10] Download and extract local TTS package"
if errorlevel 1 goto :step8_skip
echo [8/10] Downloading and extracting local TTS package...
"%PYTHON_EXE%" "%CD%\embedded_python_bootstrap.py" "%BACKEND_DIR%\download_tts.py"
if errorlevel 1 goto :fail
:step8_skip

if not exist "%FRONTEND_DIR%\package.json" (
  echo frontend package.json not found. Step [9/10] cannot continue.
  goto :fail
)
call :confirm_step "[9/10] Run npm install in frontend"
if errorlevel 1 goto :step9_skip
echo [9/10] Running npm install in frontend...
pushd "%FRONTEND_DIR%"
npm install
if errorlevel 1 (
  popd
...
if not exist "%MC_OPERATOR_DIR%\package.json" (
  echo mc-operator package.json not found. Step [10/10] cannot continue.
  goto :fail
)
call :confirm_step "[10/10] Run npm install in mc-operator"
if errorlevel 1 goto :step10_skip
echo [10/10] Running npm install in mc-operator...
...
if errorlevel 1 (
  echo pip is not available in .runtime. Complete step [4/10-C] first.
  exit /b 1
)
exit /b 0